...
def draw(notes: list, holds: dict):
	...
	timestamp2index = lambda x: int(np.round(x / chart_duration * chart_length))
	index2timestamp = lambda x: x / chart_length * chart_duration
	...
	def plot_note(note: Note):
		...
		try:
			if note.size == 60:
				...
				if note.position < 15:
					if note.position + note.size > 15+1:
						wraparound[channel, index] += 1
				else:
					if note.position + note.size > 15+60+1:
						wraparound[channel, index] += 1
